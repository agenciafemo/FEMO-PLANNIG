...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const { error } = await admin.rpc(
      "google_business_server_create_oauth_state",
      {
        _organization_id: organizationId,
        _requested_by: actor.userId,
        _state_hash: await sha256Hex(state),
        _requested_scopes: GOOGLE_BUSINESS_SCOPES,
        _redirect_path: redirectPath,
        _expires_at: new Date(Date.now() + 10 * 60 * 1000).toISOString(),
      },
    );
    if (error) throw new HttpError(500, "google_business_oauth_state_create_failed");

    return jsonResponse(
      { authorize_url: buildGoogleBusinessAuthorizeUrl(state) },
      200,
      headers,
    );
  } catch (error) {
