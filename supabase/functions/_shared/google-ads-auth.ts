...
export type GoogleAdsActor = {
  userId: string;
  email: string | null;
};

export async function requireGoogleAdsActor(
  request: Request,
): Promise<GoogleAdsActor> {
...
export async function requireGoogleAdsMembership(
  ...
  if (!data || data.status !== "active") {
    throw new HttpError(403, "google_ads_access_forbidden");
  }
  if (managerOnly && !MANAGER_ROLES.has(data.role)) {
    throw new HttpError(403, "google_ads_management_forbidden");
  }
}
