...
Deno.serve(async (request) => {
  ...
  try {
    if (rawState.length < 32 || rawState.length > 256) {
      throw new HttpError(400, "google_ads_oauth_state_missing");
    }
    const admin = createAdminClient();
    const { data, error } = await admin.rpc(
      "google_ads_server_consume_oauth_state",
      { _state_hash: await sha256Hex(rawState) },
    );
    if (error || !data?.[0]) {
      throw new HttpError(400, "google_ads_oauth_state_invalid_or_expired");
    }
    state = data[0] as OAuthState;

    if (query.get("error")) {
      return redirectTarget(returnOrigin, state.redirect_path, {
        google_ads_status: "error",
        reason_code: "google_ads_oauth_denied_by_user",
      });
    }
    const code = query.get("code");
    if (!code) throw new HttpError(400, "google_ads_authorization_code_missing");

    const token = await exchangeGoogleAdsCode(code);
    const identity = await getGoogleAdsIdentity(token.accessToken);
    const { error: saveError } = await admin.rpc(
      "google_ads_server_upsert_connection",
      {
    ...
    if (saveError) {
      const reason = saveError.message.includes("refresh_token_missing")
        ...
        : "google_ads_connection_save_failed";
      throw new HttpError(500, reason);
    }

    return redirectTarget(returnOrigin, state.redirect_path, {
      google_ads_status: "connected",
    });
  } catch (error) {
    const reason = error instanceof HttpError
      ...
      ? error.reasonCode
      : "google_ads_oauth_callback_failed";
    return redirectTarget(returnOrigin, state?.redirect_path ?? "/", {
      google_ads_status: "error",
      reason_code: reason,
    });
