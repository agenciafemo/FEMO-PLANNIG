...
// A malha inteira é redesenhada a cada quadro. Sem teto, uma tela grande vira
// dezenas de milhares de arcos por segundo — daí o limite de densidade.
const MAX_DPR = 2;
...
const THEMES: Record<string, GridTheme> = {
  ...
  // Petróleo da marca Norteia sobre grafite, em vez do azul genérico.
  brand: {
    bg: "#14171A",
    lineActive: { r: 90, g: 163, b: 158, a: 0.9 },
    nodeActive: { r: 122, g: 191, b: 186, a: 1.0 },
    glow: "90,163,158",
    ripple: "122,191,186",
  },
};
...
export default function KineticGrid({
  ...
  useEffect(() => {
    ...
    const setSize = () => {
      ...
      // Sem escalar pelo devicePixelRatio a malha sai borrada em tela retina —
      // e esta é a primeira tela que alguém vê no produto.
      const dpr = Math.min(window.devicePixelRatio || 1, MAX_DPR);
      canvas.width = Math.floor(w * dpr);
      canvas.height = Math.floor(h * dpr);
      canvas.style.width = `${w}px`;
      canvas.style.height = `${h}px`;
      // Definir width/height zera a transformação, então a escala é reaplicada.
    ...
    // Quem pediu menos movimento recebe a malha parada: um laço de canvas
    // rodando sem parar é desconforto para uns e bateria para todos.
  ...
  return (
    <div
      className={cn(
        ...
        globalColor === "monochrome" ? "bg-[#000000]"
          : globalColor === "brand" ? "bg-[#14171A]"
          : "bg-[#161618]",
        className,
      )}
    >
